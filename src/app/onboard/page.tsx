...
function Link({ handle, show, next }: StageProps) {
    ...
    // localStorage States
    const [discord, setDiscord] = useState<any>(undefined);
    const [github, setGithub] = useState<any>(undefined);
    const [twitch, setTwitch] = useState<any>(undefined);
    const [twitter, setTwitter] = useState<any>(undefined);
    const [linkedin, setLinkedin] = useState<any>(undefined);
    const [userInfoLocal, setUserInfoLocal] = useState<any>(undefined);

    useEffect(() => {
        try {
            setDiscord(localStorage.getItem("discord"));
            setGithub(localStorage.getItem("github"));
            setTwitch(localStorage.getItem("twitch"));
            setTwitter(localStorage.getItem("twitter"));
            setLinkedin(localStorage.getItem("linkedin"));
            setUserInfoLocal(localStorage.getItem("userInfo"));
        } catch (e) {
            console.error(e);
        }
